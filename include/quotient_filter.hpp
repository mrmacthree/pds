...
namespace pds {
template <class Key, class Hash = pds::hash::default_hash<Key>, class Container = std::vector<unsigned int>>
...
public:
    ...
    quotient_filter(size_t num_bits_quotient, size_t num_bits_remainder, Seed seed) : _table(std::pow(2,num_bits_quotient),0), _seed{seed} {
        assert(num_bits_quotient+num_bits_remainder == std::numeric_limits<hash_type>::digits, "q+r must equal num bits in underlying type");
    }
    ...
    void insert(const Key& key) {
        ...
        if (!quotient_seen && !_is_shifted[quotient]) {
            ...
            auto i = (_bucket_occupied[quotient] && !_is_shifted[quotient]) ? quotient : find_run(quotient);
