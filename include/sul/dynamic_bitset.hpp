...
class dynamic_bitset {
 ...
 public:
   ...
   *
   * @details    As the bits in the @ref sul::dynamic_bitset class are stored in
   * an optimized way in blocks, it is not possible for the subscript operators
   * to return a reference to a boolean. Hence this class is used as a proxy to
   * enable subscript operator of the @ref sul::dynamic_bitset class to be used
   * as if it was an array of bools.
   *
   * @since      1.0.0
   */
  class reference {
   public:
     ...
     * sul::dynamic_bitset and a bit position.
     *
  ...
  /**
   * @brief      Constructs a @ref sul::dynamic_bitset of @p nbits bits from an
   * initial value.
   *
   * @details    The first bits are initialized with the bits from @p init_val,
   * if @p nbits \> std\::numeric_limits\<unsigned long long\>\::digits , all
   * other bits are initialized to @a false. A copy of @p allocator will be used
   ...
   */
  constexpr explicit dynamic_bitset(
      size_type nbits, unsigned long long init_val = 0,
      const allocator_type& allocator = allocator_type());

  /**
   * @brief      Constructs a @ref sul::dynamic_bitset using @p init_vals to
   * initialize the first blocks.
   *
   * @details    The size of the newly created @ref sul::dynamic_bitset is @p
   * init_vals.size() *
   *             @ref bits_per_block. A copy of @p allocator will be used for
   * memory management.
   *
  ...
  /**
   * @brief      Constructs a @ref sul::dynamic_bitset from a string or a part
   * of a string.
   *
   * @details    Construct the @ref sul::dynamic_bitset using the characters
   ...
   *
   * @details    Decrease the size of the @ref sul::dynamic_bitset by one, does
   * nothing if the
   ...
   * sul::dynamic_bitset.
   *
   * @details    Increase the size of the @ref sul::dynamic_bitset by @ref
   * bits_per_block.
   *
   ...
   *
   * @remark     Return @a true if the @ref sul::dynamic_bitset is empty, the
   * logic is that you are checking if all bits are set to @a true, meaning none
   * of them is set to @a false, and in an empty @ref sul::dynamic_bitset no
   ...
   * false
   *
   * @remark     Return @a false if the @ref sul::dynamic_bitset is empty, the
   ...
   * empty @ref sul::dynamic_bitset there is no bit that can be set to @a true.
   *
   * @complexity Linear in the size of the @ref sul::dynamic_bitset.
   *
   * @since      1.0.0
   ...
   *             @endcode
   *
   * @return     @a true if the @ref sul::dynamic_bitset is empty, @a false
   * otherwise
   *
   * @complexity Constant.
   *
  ...
  /**
   * @brief      Give the number of bits that the @ref sul::dynamic_bitset has
  ...
  /**
   * @brief      Increase the capacity of the @ref sul::dynamic_bitset to a
   ...
   * sul::dynamic_bitset are invalidated, otherwise the method does nothing.
   *
   ...
   * sul::dynamic_bitset are invalidated.
   *
   * @complexity At most linear in the size of the @ref sul::dynamic_bitset.
   *
   * @since      1.0.0
  ...
  /**
   * @brief      Determines if this @ref sul::dynamic_bitset is a subset of @p
   * bitset.
   *
   * @details    This @ref sul::dynamic_bitset is a subset of @p bitset if, for
   * every bit that is set in this @ref sul::dynamic_bitset, the corresponding
   ...
   *             @endcode
   *
   * @param[in]  bitset  The @ref sul::dynamic_bitset for which to check if this
   * @ref sul::dynamic_bitset is a subset
   *
   * @return     @a true if this @ref sul::dynamic_bitset is a subset of @p
  ...
  /**
   * @brief      Determines if this @ref sul::dynamic_bitset is a proper subset
   * of @p bitset.
   *
   * @details    This @ref sul::dynamic_bitset is a proper subset of @p bitset
   * if, for every bit that is set in this @ref sul::dynamic_bitset, the
   * corresponding bit in @p bitset a is also set and if this @ref
   * sul::dynamic_bitset is different from @p bitset.\n\n Less efficient but
   ...
   *
   * @param[in]  bitset  The @ref sul::dynamic_bitset for which to check if this
   * @ref sul::dynamic_bitset is a proper subset
   *
   * @return     @a true if this @ref sul::dynamic_bitset is a proper subset of
  ...
  /**
   * @brief      Determines if this @ref sul::dynamic_bitset and @p bitset
   * intersect.
   *
   * @details    This @ref sul::dynamic_bitset intersects with @p bitset if for
   * at least one bit set in this @ref sul::dynamic_bitset, the corresponding
   ...
   *
   * @param[in]  bitset  The @ref sul::dynamic_bitset for which to check if this
   * @ref sul::dynamic_bitset intersects
   *
   * @return     @a true if this @ref sul::dynamic_bitset intersects with @p
   * bitset, @a false otherwise
   *
   * @pre        @code
   *             size() == bitset.size()
   ...
   * sul::dynamic_bitset starting from the least-significant bit.
   *
   * @details    Give the lowest index of the @ref sul::dynamic_bitset with a
   ...
   * 1, @ref size()\[ of the @ref sul::dynamic_bitset starting from the position
   * @p prev + 1.
   *
   * @details    Give the lowest index superior to @p prev of the @ref
   * sul::dynamic_bitset with a bit set, or @ref npos if no bits are set after
  ...
  /**
   * @brief      Exchanges the bits of this @ref sul::dynamic_bitset with those
   * of @p other.
   *
   * @details    All @ref reference on bits of the @ref sul::dynamic_bitset are
   * invalidated.
   *
   ...
   * @since      1.0.0
   */
  constexpr void swap(dynamic_bitset<Block, Allocator>& other) noexcept(
  ...
  /**
   * @brief      Converts the contents of the bitset to an <tt>unsigned
   * long</tt> integer.
   *
   * @details    The first bit of the bitset corresponds to the least
   * significant digit of the number and the last bit corresponds to the most
   * significant digit.
   *
   * @return     The numeric value corresponding to the bitset contents.
   *
   * @throws     std::overflow_error  if the value is too large to be
   * represented in an <tt>unsigned long</tt>
   *
   * @complexity Constant.
   *
   * @since      1.3.0
  ...
  /**
   * @brief      Converts the contents of the bitset to an <tt>unsigned long
   * long</tt> integer.
   *
   * @details    The first bit of the bitset corresponds to the least
   * significant digit of the number and the last bit corresponds to the most
   ...
   *
   * @throws     std::overflow_error  if the value is too large to be
   * represented in an <tt>unsigned long long</tt>
   *
   * @complexity Constant.
   *
   * @since      1.3.0
  ...
  /**
   * @brief      Iterate on the @ref sul::dynamic_bitset and call @p function
   ...
   *
   *
   * @post       The bits past the end of the @ref sul::dynamic_bitset in the
   ...
   *             @endcode
   *
   * @remark     If the @ref sul::dynamic_bitset is empty, this function may or
   ...
   *
   * @param[in]  lhs         The left hand side @ref sul::dynamic_bitset of the
   * operator
   * @param[in]  rhs         The right hand side @ref sul::dynamic_bitset of the
   * operator
   *
   ...
   *             sul::dynamic_bitset as a container and not a number. For
   ...
   * used as a secondary criteria for the comparison of @ref sul::dynamic_bitset
   * which content represent the same number. Therefore, for the previous
   * example with bitsets A(0011) and B(011), B \< A is @a true, A \<= B is @a
   * false, B \<= A is @a true and A == B is @a false.\n\n If comparing bitsets
   ...
   *             @endcode
   *
   * @remark     The empty @ref sul::dynamic_bitset is the "lowest" of all
 ...
 * of the two @ref sul::dynamic_bitset is first on numbers their content
 ...
 * two @ref sul::dynamic_bitset is first on numbers their content represent and
 * then on their size.
 *
 * @details    Defined as:
 *             @code
 ...
 * comparison of the two @ref sul::dynamic_bitset is first on numbers their
 ...
 * management
 *
 * @return     A @ref sul::dynamic_bitset with each bit being the result of a
 ...
 * management
 *
 * @return     A @ref sul::dynamic_bitset with each bit being the result of the
 ...
 * with @ref sul::dynamic_bitset::to_string() with default parameter, using '1'
...
/**
 * @brief      Extract a @ref sul::dynamic_bitset from a character stream using
...
constexpr void swap(dynamic_bitset<Block, Allocator>& bitset1,
                    dynamic_bitset<Block, Allocator>&
                        bitset2) noexcept(noexcept(bitset1.swap(bitset2)));
...
template <typename Block, typename Allocator>
constexpr dynamic_bitset<Block, Allocator>::dynamic_bitset(
    size_type nbits, unsigned long long init_val,
    const allocator_type& allocator)
...
constexpr void dynamic_bitset<Block, Allocator>::swap(
    dynamic_bitset<Block, Allocator>&
        other) noexcept(noexcept(std::swap(m_blocks, other.m_blocks))) {
...
constexpr unsigned long dynamic_bitset<Block, Allocator>::to_ulong() const {
  ...
  for (size_type i_block = 0; i_block <= block_index(result_bits_number - 1);
       ++i_block) {
    result |= (static_cast<unsigned long>(m_blocks[i_block])
               << (i_block * bits_per_block));
  }
...
template <typename Block, typename Allocator>
constexpr unsigned long long dynamic_bitset<Block, Allocator>::to_ullong()
    const {
  if (m_bits_number == 0) {
    return 0;
  }
  ...
  for (size_type i_block = 0; i_block <= block_index(result_bits_number - 1);
       ++i_block) {
    result |= (static_cast<unsigned long long>(m_blocks[i_block])
               << (i_block * bits_per_block));
  }
...
dynamic_bitset<Block, Allocator>::block_count(
  ...
  if constexpr (bits_per_block <= u_bits_number) {
    return static_cast<size_type>(
        __builtin_popcount(static_cast<unsigned int>(block)));
  } else if constexpr (bits_per_block <= ul_bits_number) {
    return static_cast<size_type>(
        __builtin_popcountl(static_cast<unsigned long>(block)));
  } else if constexpr (bits_per_block <= ull_bits_number) {
    return static_cast<size_type>(
        __builtin_popcountll(static_cast<unsigned long long>(block)));
  }
#endif
...
dynamic_bitset<Block, Allocator>::block_count(const block_type& block,
  ...
  if constexpr (bits_per_block <= u_bits_number) {
    return static_cast<size_type>(
        __builtin_popcount(static_cast<unsigned int>(shifted_block)));
  } else if constexpr (bits_per_block <= ul_bits_number) {
    return static_cast<size_type>(
...
dynamic_bitset<Block, Allocator>::count_block_trailing_zero(
  ...
  if constexpr (bits_per_block <= u_bits_number) {
    return static_cast<size_type>(
        __builtin_ctz(static_cast<unsigned int>(block)));
  } else if constexpr (bits_per_block <= ul_bits_number) {
    return static_cast<size_type>(
        __builtin_ctzl(static_cast<unsigned long>(block)));
  } else if constexpr (bits_per_block <= ull_bits_number) {
    return static_cast<size_type>(
        __builtin_ctzll(static_cast<unsigned long long>(block)));
  }
