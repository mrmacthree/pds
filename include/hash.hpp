...
concept HashGenerator = requires (T t, size_t n, Key k) {
    ...
    requires std::same_as<std::ranges::range_value_t<decltype(t.hashes(k))>, typename T::hash_type>;
} && std::unsigned_integral<typename T::hash_type>;
...
concept HashFunction = requires (T t) {
    ...
    std::unsigned_integral<typename T::hash_type>;
    requires requires (Key& k, T::seed_type s) {{t.operator()(k,s)} -> std::same_as<typename T::hash_type>;};
};
...
public:
    ...
    simple_hash_generator(size_t hashes_per_key) : _hashes_per_key(hashes_per_key)
    {}
    auto hashes(const Key& key) const {
        return std::views::iota(0u, _hashes_per_key) | std::views::transform([&key](seed_type seed){
            return Hash{}(key,seed);
        });
    }
    
    size_t hashes_per_key() const {
        return _hashes_per_key;
    }
...
template <typename Key, HashFunction<Key> Hash, typename Allocator = std::allocator<typename Hash::seed_type>>
